...
DataAccessObject.create = function (data, callback) {
  if (deferInvocation(this.getDataSource(), this, this.create, arguments)) {
    return;
  }
  ...
  if (Array.isArray(data)) {
    // Invoke create() for each of the items
    var instances = [];
...
function deferInvocation(dataSource, obj, method, args) {
  return dataSource.deferInvocation(obj, method, args);
}
...
DataAccessObject.updateOrCreate = DataAccessObject.upsert = function upsert(data, callback) {
  if (deferInvocation(this.getDataSource(), this, this.upsert, arguments)) {
    return;
  }
...
DataAccessObject.exists = function exists(id, cb) {
  if (deferInvocation(this.getDataSource(), this, this.exists, arguments)) {
    return;
  }
...
DataAccessObject.findById = function find(id, cb) {
  if (deferInvocation(this.getDataSource(), this, this.findById, arguments)) {
    return;
  }

  var self = this;
  this.getDataSource().connector.find(self.modelName, id, function (err, data) {
    var obj = null;
    if (data) {
      if (!getIdValue(self, data)) {
        setIdValue(self, data, id);
      }
      obj = new self(data, {applySetters: false});
    }
    cb(err, obj);
  });
};
...
DataAccessObject.find = function find(query, cb) {
  if (deferInvocation(this.getDataSource(), this, this.find, arguments)) {
    return;
  }
...
DataAccessObject.findOne = function findOne(query, cb) {
  if (deferInvocation(this.getDataSource(), this, this.findOne, arguments)) {
    return;
  }
...
DataAccessObject.remove = DataAccessObject.deleteAll = DataAccessObject.destroyAll = function destroyAll(where, cb) {
      if (deferInvocation(this.getDataSource(), this, this.destroyAll, arguments)) {
        return;
      }
      var Model = this;
      ...
      if (!where) {
        ...
        this.getDataSource().connector.destroyAll(Model.modelName, where, function (err, data) {
          cb && cb(err, data);
          if(!err) {
            Model.emit('deletedAll', where);
          }
        });
      }
    };
...
// compatible for angular SDK
DataAccessObject.removeById = DataAccessObject.destroyById
  = DataAccessObject.deleteById = function deleteById(id, cb) {
  if (deferInvocation(this.getDataSource(), this, this.deleteById, arguments)) {
    return;
  }
  var Model = this;

  this.getDataSource().connector.destroy(Model.modelName, id, function (err) {
...
DataAccessObject.count = function (where, cb) {
  if (deferInvocation(this.getDataSource(), this, this.count, arguments)) {
    return;
  }
...
DataAccessObject.prototype.save = function (options, callback) {
  if (deferInvocation(this.getDataSource(), this, this.save, arguments)) {
    return;
  }
...
DataAccessObject.updateAll = function (where, data, cb) {
  if (deferInvocation(this.getDataSource(), this, this.updateAll, arguments)) {
    return;
  }
...
DataAccessObject.prototype.remove =
  DataAccessObject.prototype.delete =
    DataAccessObject.prototype.destroy = function (cb) {
      if (deferInvocation(this.getDataSource(), this, this.destroy, arguments)) {
        return;
      }
      var Model = this.constructor;
      ...
      this.trigger('destroy', function (destroyed) {
        this._adapter().destroy(Model.modelName, id, function (err) {
          if (err) {
            return cb(err);
          }
...
DataAccessObject.prototype.updateAttributes = function updateAttributes(data, cb) {
  if (deferInvocation(this.getDataSource(), this, this.updateAttributes, arguments)) {
    return;
  }
...
DataAccessObject.prototype.reload = function reload(callback) {
  if (deferInvocation(this.getDataSource(), this, this.reload, arguments)) {
    return;
  }
 ...
 * the declaring model
 */
DataAccessObject.scope = function (name, query, targetClass, methods) {
  defineScope(this, targetClass || this, name, query, methods);
};
